...
import com.examly.springapp.SpringRepository;
...
public class SpringController {
    @Autowired
    SpringRepository taskRepository;


    @PostMapping("/saveTask")
    public ResponseEntity<?> saveTask(@RequestBody Springapp task){
    ...
    public ResponseEntity<?> deleteById(@PathVariable String taskId){
        List<Springapp> allTask = (List<Springapp>) taskRepository.findAll();

        if(allTask !=null){
            for(Springapp task : allTask){
